...
//    private static final String CLASS_OF_PROXY_CORE_SERVER = "com.alibaba.jvm.sandbox.core.server.ProxyCoreServer";
    private static final String JVM_SANDBOX = "com.alibaba.jvm.sandbox.core.CustomSandbox";
    ...
    public static synchronized void uninstall(final String namespace) throws Throwable {
        ...
        // 关闭服务器
        final Class<?> classOfSandbox = sandboxClassLoader.loadClass(JVM_SANDBOX);
        classOfSandbox.getMethod("destroy")
                .invoke(classOfSandbox.getMethod("getInstance").invoke(null));
    ...
    private static synchronized void install(final Map<String, String> featureMap,
        ...
        try {
            ...
            // 获取CoreServer单例
            final Object objectOfSandbox = classOfSandbox
                    .getMethod("getInstance", classOfConfigure, Instrumentation.class)
                    .invoke(null, objectOfCoreConfigure, inst);

            try {
                classOfSandbox
                        .getMethod("instrument")
                        .invoke(objectOfSandbox);
            } catch (Throwable t) {
                throw t;
            }
